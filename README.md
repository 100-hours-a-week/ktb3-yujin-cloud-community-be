...
그리고 AWS EC2·RDS·ALB를 통한 배포 자동화**까지
...
## 👩‍💻 핵심 개선 포인트
1. 게시물 조회 시 N+1 문제 해결
    - 게시물 목록 조회에서 작성자 프로필, 좋아요 수, 댓글 수를 각각 따로 조회하면서 전형적인 N+1 문제가 발생했습니다.
    - 이를 해결하기 위해 QueryDSL 기반 batch 조회(IN 절) 방식으로 재구성하여 한 번의 쿼리로 필요한 데이터를 모두 가져오도록 최적화했습니다.
2. S3 이미지 업로드를 Presigned URL 방식으로 전환
    - 초기에는 이미지를 BE 서버로 전달받은 후 S3로 업로드하는 구조였지만, BE가 multipart 처리·파일 업로드까지 모두 담당해야 해 부하가 컸습니다.
    - Presigned URL로 전환해 FE → S3 직접 업로드 방식으로 개선하여 백엔드의 I/O 부담을 줄이고 응답 속도도 개선했습니다.
3. JWT 인증/인가 구조 정립
    - 로그인·토큰 재발급·로그아웃 흐름을 구현하면서 쿠키 기반(HttpOnly) 인증 전략과 DB 저장형 Refresh Token 구조를 적용해
      보안성과 실 서비스 흐름을 동시에 확보한 JWT 인증 체계를 구축했습니다.

## 🌐 서버 아키텍처
> [👉 전체 아키텍처 설계 과정 보러가기](https://velog.io/@yujin22/ClosetLoungeNetwork)
...
## ▶️ 시연 영상
> [YouTube로 보러가기](https://youtu.be/9KiUMCJ4E7g)
...
## Frontend 보기
[Frontend Github 보러가기](https://github.com/Yu-Jin22/ktb3-yujin-cloud-community-fe) 
